...
use std::{fs, fs::File, io::Write, path::Path, time::Instant};
...
fn prove_and_verify_benchmark(c: &mut Criterion) {
    ...
    for input_file in candidate_files {
        ...
        for sample in 0..sample_size {
            ...
            assert!(
                input.metadata.num_states_enumerated != 0,
                "Must have states!"
            );
            for state in input.embeddings.E_S0.iter() {
                assert!(
                    *state < input.metadata.num_states_enumerated as u64,
                    "S_0 must be in valid states!"
                );
            }
            for e in input.embeddings.E_init.iter() {
                assert!(
                    *e < input.metadata.num_states_enumerated as u64,
                    "e in E_init must be valid states!"
                );
            }

            assert!(
                input.metadata.num_transitions_checked != 0,
                "Must have states!"
            );
            for state in input.embeddings.E_init.iter() {
                assert!(
                    *state < input.metadata.num_states_enumerated as u64,
                    "T must be in valid transitions!"
                );
            }
            for e in input.embeddings.E_step.iter() {
                assert!(
                    *e < input.metadata.num_transitions_checked as u64,
                    "e in E_step must be valid transition!"
                );
            }
            for e in input.embeddings.E_fairstep.iter() {
                assert!(
                    *e < input.metadata.num_transitions_checked as u64,
                    "e in E_fairstep must be valid transition!"
                );
            }
            ...
            if proof_opt.is_none() {
                ...
                if verified_opt.is_none() {
                    writeln!(
                        log_file,
                        "Sample no. {:?} -- Setup: {:?}ms -- Prover: {:?}ms -- Verifier OOT.",
                        sample, setup_time, prove_time_min_setup
                    )
                    .expect("Error writing to log file");
                } else {
