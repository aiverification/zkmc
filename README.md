...
- [`encoder/`](encoder/) — Python toolkit (`zkterm`, `zkrank`, `zkverify`, `zkfarkas`, `zkexplicit`, `zkltl`, `zksynth`, `zkits`, `zksmv`, `zkhypersmv`). Encodes guarded commands and ranking functions into matrix/vector form, discharges termination obligations via Farkas' lemma + Z3, and exports JSON consumable by the prover. Programs can be written as `.gc`, imported from KoAT `.koat`, or imported from the supported SMV subset. `zkhypersmv` glues `.smv + .hq` together by lowering SMV, reducing supported universal HyperLTL via self-composition, deriving the bad-run automaton via [Spot](https://spot.lre.epita.fr/), and emitting explicit ZKMC JSON. See [`encoder/README.md`](encoder/README.md) and [`encoder/LANGUAGE.md`](encoder/LANGUAGE.md).
