...
- [`encoder/`](encoder/) — Python toolkit (`zkterm`, `zkrank`, `zkverify`, `zkfarkas`, `zkexplicit`, `zkltl`). Encodes guarded commands and ranking functions into matrix/vector form, discharges termination obligations via Farkas' lemma + Z3, and exports JSON consumable by the prover. Properties can be written directly as a Büchi automaton or in LTL (`spec:`), with the automaton derived via [Spot](https://spot.lre.epita.fr/) (`ltl2tgba`, an optional external tool). See [`encoder/README.md`](encoder/README.md) and [`encoder/LANGUAGE.md`](encoder/LANGUAGE.md).
