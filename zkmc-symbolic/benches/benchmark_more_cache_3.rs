...
fn prove_and_verify_benchmarks_full_cache_cached(c: &mut Criterion) {
    ...
    for input_file in candidate_files {
        ...
        for sample in 0..sample_size {
            ...
            let bp_gens = BulletproofGens::new(64, max_l + 1);
            ...
            let b_verify: Vec<BVerifyEntry> = prover_b_cache
                ...
                .collect();
            let lambda_verify: Vec<ZKRPVerifyEntry> = prover_lambda_cache
                .iter()
                .map(|(ls, l)| ZKRPVerifyEntry {
                    commitment: l.lambda_blind,
                    zkrp_proof: l.lambda_zkrp.clone(),
                    m: ls.len(),
                    n: 1,
                    l: ls.len() * ls[0].len(),
                })
                .collect();
            let mu_verify: Vec<ZKRPVerifyEntry> = prover_mu_cache
                .iter()
                .map(|(ms, m)| ZKRPVerifyEntry {
                    commitment: m.mu_blind,
                    zkrp_proof: m.mu_zkrp.clone(),
                    m: ms.len(),
                    n: 1,
                    l: ms.len() * ms[0].len(),
                })
                .collect();
            let e1_verify: Vec<ZkmmVerifyEntry> = prover_e1_cache
                .iter()
                .map(|((A_s_T, lambda_s, q), e1)| ZkmmVerifyEntry {
                    c_a: prover_A_cache.get(&(A_s_T.clone(), *q)).unwrap().A_blind,
                    c_e: e1.e1_blind,
                    c_lambda: prover_lambda_cache.get(lambda_s).unwrap().lambda_blind,
                    proof: e1.zkmm_proof.clone(),
                    m: A_s_T.len(),
                    n: 1,
                    l: A_s_T[0].len(),
                })
                .collect();
            let e2_verify: Vec<ZkmmVerifyEntry> = prover_e2_cache
                .iter()
                .map(|((neg_b_s_T, lambda_s, q), e2)| ZkmmVerifyEntry {
                    c_a: prover_b_cache.get(&(neg_b_s_T.clone(), *q)).unwrap().b_blind,
                    c_e: e2.e2_blind,
                    c_lambda: prover_lambda_cache.get(lambda_s).unwrap().lambda_blind,
                    proof: e2.zkmm_proof.clone(),
                    m: 1,
                    n: 1,
                    l: neg_b_s_T[0].len(),
                })
            ...
            println!(
                "Phase 2.5b: Pre-verifying {} unique proofs (par_iter)...",
            ...
            e1_verify.par_iter().for_each(|entry| {
                ...
                verifier_e1_cache.insert(
                    (
                        ...
                        HashableGtElement(entry.c_lambda),
                        HashableGtElement(entry.c_e),
                    ),
                    (),
                );
            });
